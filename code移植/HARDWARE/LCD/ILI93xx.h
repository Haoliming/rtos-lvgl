...
//4.3寸电容屏  LCD_ID:5510  TOUCH_id: 9147
/*
FMSC_NE1  --->  FMSC_Bank1_NORSRAM1  
FMSC_NE2  --->  FMSC_Bank1_NORSRAM2
FMSC_NE3  --->  FMSC_Bank1_NORSRAM3
FMSC_NE4  --->  FMSC_Bank1_NORSRAM4

LCD_RS--->FMSC_A16     PD11
LCD_WR--->FMSC_NWE     PD5
LCD_CS--->FMSC_NE1     PD7
LCD_RD--->FMSC_NOE     PD4

SDA(T_MOSI)  PA1 电容触摸屏信号      
CS(CT_RST)   PA2 电容触摸屏复位信号
SCL(T_SCK)	 PA5 电容触摸屏clk信号
MISO()		     未用到
PEN(CT_INT)  PA4 电容触摸中断信号
*/
...
//L2R_U2D是原子默认的设置 在GUI触摸检测不用更改 默认xvalue=tp_dev.x[0]; yvalue = tp_dev.y[0];

//R2L_D2U是我自己的设置 在GUI触摸检测 改为  xvalue=lcddev.width-tp_dev.x[0];  yvalue = lcddev.height-tp_dev.y[0];
